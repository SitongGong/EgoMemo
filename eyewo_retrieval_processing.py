...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default=os.environ.get("EYEWO_DATA_PATH", "./data/eyewo/estp_bench_sq.json"),        # "/data/gst/dataset/egolife/A1_JAKE"
    ...
    parser.add_argument("--root_path", type=str, default=os.environ.get("RESULTS_ROOT", "./results") + "/eyewo_results_question_1")
    ...
    for video_name, clip_info in video_data.items():    # 单独处理每一个视频，使用idx区分同一个视频的不同处理实例
        for clip_name, clip_list in clip_info.items():
            if clip_name == "goalstep":
                ...
                # assert len(clip_info["goalstep"]) == 1
                for idx, clip_list in enumerate(clip_info["goalstep"]):
                    ...
                    # 检查是否已存在retrieval_history.json
                    retrieval_history_path = f"{args.root_path}/{clip_name}_{args.caption_model_name}/retrieval_history.json"
                    if os.path.exists(retrieval_history_path):
                        # 读取已存在的retrieval_history.json并融合到results_dict
                        try:
                            with open(retrieval_history_path, 'r') as f:
                                retrieval_history = json.load(f)

                            # 如果retrieval_history为空，继续处理而不是跳过
                            if not retrieval_history:
                                logger.warning(f"{clip_name} 的retrieval_history.json为空，将重新处理")
                            else:
                                ...
                                results_dict[clip_name] = {
                                    ...
                                    'clip_duration': clip_duration,
                                    'task_type': task_type,
                                    'query_results': retrieval_history.get('query_results', retrieval_history)
                                }
                                logger.info(f"已加载 {clip_name} 的retrieval_history.json")
                                continue
                        except Exception as e:
                            logger.error(f"读取 {retrieval_history_path} 失败: {e}")
                    ...
                    # output_result.append((task_type, user_query_list))
                    # continue
                ...
                for question_pair in clip_list:
                    task_type.append(question_pair["Task Type"])
    ...
    if results_dict:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False, dir='/tmp') as tmp_f:
                ...
                json.dump(results_dict, tmp_f, indent=2, ensure_ascii=False)
            ...
            logger.info(f"\n{'='*60}")
            logger.info(f"所有视频的检索结果已保存到: {all_results_file}")
            logger.info(f"总共包含 {len(results_dict)} 个视频的检索结果")
            logger.info(f"{'='*60}\n")
        except Exception as e:
            logger.error(f"保存所有检索结果失败: {e}")
            # Try to save without indentation to reduce size
            try:
                with open(all_results_file, 'w', encoding='utf-8') as f:
                    json.dump(results_dict, f, ensure_ascii=False)
                logger.info(f"所有视频的检索结果已保存到（无格式化）: {all_results_file}")
            except Exception as e2:
                logger.error(f"保存所有检索结果失败（第二次尝试）: {e2}")