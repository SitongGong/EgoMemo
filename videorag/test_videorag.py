...
class VideoGraph:
    ...
    def streaming_graph_construction(self, frame_time_data, window_seconds=10, 
                       gap_threshold_seconds=60, max_new_tokens=512, test_query=None):
        """
        简化的流式建图和检索测试函数
        ...
        Args:
            ...
            window_seconds: Window size in seconds for caption generation (default: 10)
            gap_threshold_seconds: Gap threshold to start new window (default: 60)
            max_new_tokens: Maximum number of tokens to generate (default: 512)
            test_query: Optional query string for retrieval test. If None, uses default query.
            
        Returns:
            Dictionary with generated captions and retrieval result:
            {
                "second_captions": {time_span: caption, ...},
                "retrieval_result": str  # Retrieval response
            }
        """
        if not frame_time_data:
            logger.warning("No frames provided for caption generation")
            return {"second_captions": {}, "retrieval_result": None}
        
        loop = always_get_an_event_loop()
        
        # Default prompt for 10s captions
        default_prompt_second = PROMPTS["second_caption_user_prompt"]
        ...
        logger.info(f"Starting streaming processing: {len(frame_time_data)} frames, window={window_seconds}s")
                ...
                if day_changed or is_gap or window_duration >= window_seconds:
                    ...
                    if current_window_frames:
                        ...
                        try:
                            ...
                            loop.run_until_complete(self.video_segments.upsert({time_span: {"content": caption, "video_frames": current_window_frames, "type": "second"}}))
                            ...
                            loop.run_until_complete(self.ainsert_streaming_caption({time_span: {"content": caption, "video_frames": current_window_frames, "type": "second"}}))
        ...
        if current_window_frames:
            ...
            assert start_day == end_day, "Start day and end day should be the same"
            ...
            try:
                ...
                # 将caption信息和视频帧存入到video_segment和video_segment_feature_vdb中
                loop.run_until_complete(self.video_segments.upsert({time_span: {"content": caption, "video_frames": current_window_frames, "type": "minute"}}))
                # loop.run_until_complete(self.video_segment_feature_vdb.upsert_video_segment(time_span, current_window_frames))
                
                loop.run_until_complete(self._save_video_segments())
                
                # 根据caption进行流式建图
                loop.run_until_complete(self.ainsert_streaming_caption({time_span: {"content": caption, "video_frames": current_window_frames, "type": "minute"}}))
        ...
        if test_query is None:
            # 如果没有提供query，使用默认query：从生成的captions中提取关键词
            if second_captions:
                # 使用第一个caption作为测试query
                first_caption = list(second_captions.values())[0]
                test_query = f"请描述视频中的主要内容：{first_caption[:100]}"
            else:
                test_query = "请描述视频中的主要内容"
        
        logger.info(f"Executing retrieval test with query: {test_query}")
        try:
            from .base import QueryParam
            query_param = QueryParam(mode="videorag")
            
            retrieval_result = loop.run_until_complete(streaming_videorag_query(
                test_query,
                self.entities_vdb,
                self.text_chunks,
                self.chunks_vdb,
        ...
        logger.info(f"Completed streaming processing: generated {len(second_captions)} 10s captions")
