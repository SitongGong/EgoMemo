...
def timestamp_number_to_seconds(timestamp_number):
...
def seconds_to_timestamp_number(seconds):
...
class VideoGraphSeparated:
    ...
    def streaming_graph_construction(
        ...
        # 定义保存和加载状态的辅助函数
        def save_checkpoint_state(accumulated_captions, window_states, captions_dict=None, current_day=None):
        ...
        def load_checkpoint_state(current_day=None):
            ...
            try:
                ...
                # 检查day是否匹配
                saved_day = checkpoint_data.get("current_day", None)
                day_changed = False
                
                # 如果当前day不为None，需要检查匹配：
                # 1. 如果保存的day为None但当前day不为None，说明可能是从旧版本升级或之前处理的是非egolife数据集
                # 2. 如果保存的day和当前day都不为None但不相等，说明day变化了
                if current_day is not None:
                    if saved_day is None:
                        logger.info(f"Checkpoint has no day info but current day is {current_day}, resetting window states but keeping captions")
                        day_changed = True
                    elif saved_day != current_day:
                        logger.info(f"Day changed from {saved_day} to {current_day}, resetting window states but keeping captions")
                        day_changed = True
                ...
                if day_changed:
                    ...
                    return accumulated_captions, None, captions_dict
                ...
                # 支持2个元素（start, end）或4个元素（video_name, frame_idx, start, end）的格式
                if 'frame_time_ranges_list' in window_states_raw and isinstance(window_states_raw['frame_time_ranges_list'], list):
                    ...
                    for item in window_states_raw['frame_time_ranges_list']:
                        if isinstance(item, list) and (len(item) == 2 or len(item) == 4):
        ...
        loaded_accumulated_captions, loaded_window_states, loaded_captions_dict = load_checkpoint_state(current_day=day)
        ...
        for video_idx, video_path in enumerate(days_list):     # 读取视频和注释文件
            ...
            if datasets_type == "egolife":
                ...
                # proassist数据集的video_path是一个列表，包含已经采样好的帧（2fps，每0.5秒一帧）
                # 直接使用这个列表作为base64_frames
                base64_frames = video_path if isinstance(video_path, list) else [video_path]
                video_name = "proassist_video"  # proassist数据集没有文件名，使用默认名称
                
                # 生成frame_timestamps和frame_time_ranges
                # 2fps意味着每0.5秒一帧，但每两帧（1秒）的时间戳相同
                frame_timestamps = []
                frame_time_ranges = []
                
                time_span_info = None
                for i in range(len(base64_frames)):
                    # 每两帧共享一个时间戳（每0.5秒一帧，每两帧为1秒）
                    # 帧0和帧1使用时间戳0秒，帧2和帧3使用时间戳1秒，以此类推
                    timestamp_seconds = i // 2  # 每两帧一个时间戳
                    ...
                    # 生成时间范围：每两帧共享一个时间范围
                    # 帧0和帧1：[0, 1]秒，帧2和帧3：[1, 2]秒，以此类推
                    range_start_seconds = timestamp_seconds
                    range_end_seconds = timestamp_seconds + 1
            ...
            for i, frame in enumerate(base64_frames):
                ...
                if i < len(frame_time_ranges):
                    ...
                    # 对于proassist数据集，连续两帧可能有相同的frame_range_key，需要加上video_name和frame_idx来区分
                    if datasets_type == "proassist":
                        frame_range_key = (video_name, i, current_frame_time_range.get('start'), current_frame_time_range.get('end'))
                    else:
                                ...
                                for retry_count in range(max_retries):
                                    try:
                                        ...
                                        for frame_idx, content in caption["frames"].items():
                                            ...
                                            start_time_formatted = format_timestamp_to_hhmmsscc(frame_data['start_time'])
                                            end_time_formatted = format_timestamp_to_hhmmsscc(frame_data['end_time'])
                                            # 对于proassist数据集，fps=2，每两帧共享一个时间范围，需要添加帧编号来区分
                                            if datasets_type == "proassist":
                                                # 获取原始帧索引，计算在该时间范围内的帧编号（0或1）
                                                original_frame_idx = frame_data['frame_idx']
                                                frame_number_in_range = original_frame_idx % 2
                                                caption_key = f"DAY{day_num}-{start_time_formatted}-{end_time_formatted}-{frame_number_in_range}"
                                            else:
                ...
                try:
                    ...
                    for frame_idx, content in caption["frames"].items():
                        ...
                        if datasets_type == "proassist":
                            ...
                            original_frame_idx = frame_data['frame_idx']
                            frame_number_in_range = original_frame_idx % 2
                            caption_key = f"DAY{start_day}-{start_time_formatted}-{end_time_formatted}-{frame_number_in_range}"
                        else:
    ...
    def process_frame_with_proactive_service(self, frame_info, accumulated_captions, datasets_type, max_captions_per_level=3, history_messages=None, retrieved_memory=None):
        ...
        time_span = frame_info.get('time_span_info', {}).get("time_span")
        ...
        for idx, second_caption_info in enumerate(second_captions_list):
            ...
            for service in service_list:
                if isinstance(service, dict) and 'memory_query' in service:
                    ...
                    sub_service_type = service.get('service_sub_type', '')
    ...
    def streaming_graph_construction_with_user_queries(
        self,
        dialog_json_path,
        data_path=None,
        anno_path=None,
        ...
        try:
            ...
            logger.info(f"Loaded dialog data from {dialog_json_path}")
        except Exception as e:
            logger.error(f"Failed to load dialog JSON file {dialog_json_path}: {e}")
            raise
        
        # 提取所有用户问题和对应的时间戳
        user_queries = []  # List of (time_in_seconds, question_text)
        
        conversations = dialog_data.get('conversations', [])
        for conv in conversations:
            conversation = conv.get('conversation', [])
            for msg in conversation:
                if msg.get('role') == 'user':
                    query_time = msg.get('time', None)
                    query_content = msg.get('content', '')
                    if query_time is not None and query_content:
                        user_queries.append((query_time, query_content))
        
        # 按时间排序
        user_queries.sort(key=lambda x: x[0])
        logger.info(f"Extracted {len(user_queries)} user queries from dialog file")
        
        # 存储用户查询的响应
        user_query_responses = []
        processed_query_indices = set()  # 跟踪已处理的查询索引
        
        # 调用原有的流式建图函数，但需要修改以支持在特定时刻处理用户问题
        # 由于不能修改原有函数，我们需要复制其逻辑并添加用户问题处理
        
        # 首先调用原有的流式建图函数获取基础结果
        base_result = self.streaming_graph_construction(
            data_path=data_path,
            anno_path=anno_path,
            day=day,
            interval_seconds=interval_seconds,
        ...
        accumulated_captions = base_result.get('accumulated_captions', {
            'second_captions': [],
            'min_captions': [],
            'hour_captions': []
        })
        
        # 处理用户查询：遍历所有用户查询，找到对应的帧并进行检索
        loop = always_get_an_event_loop()
        
        # 获取所有second_captions，按时间排序
        second_captions_list = accumulated_captions.get('second_captions', [])
        if second_captions_list:
            second_captions_list = sorted(second_captions_list, key=lambda x: x.get('timestamp', 0))
        
        # 为每个用户查询找到最接近的second_caption
        for query_idx, (query_time, query_content) in enumerate(user_queries):
            if query_idx in processed_query_indices:
                continue
            
            # 找到最接近的second_caption
            best_match_idx = None
            min_time_diff = float('inf')
            
            # 辅助函数：将timestamp转换为秒数
            def timestamp_to_seconds(ts, is_egolife):
                if is_egolife:
                    # Egolife数据集：HHMMSSCC格式（8位整数）
                    ts_int = int(ts)
            ...
            for idx, caption_info in enumerate(second_captions_list):
                caption_timestamp = caption_info.get('timestamp', 0)
                # 将timestamp转换为秒数
                caption_time_seconds = timestamp_to_seconds(caption_timestamp, datasets_type == "egolife")
                
                time_diff = abs(caption_time_seconds - query_time)
                if time_diff < min_time_diff:
                    min_time_diff = time_diff
                    best_match_idx = idx
            
            # 如果找到匹配的caption且时间差在容差范围内
            if best_match_idx is not None and min_time_diff <= time_tolerance_seconds:
                matched_caption_info = second_captions_list[best_match_idx]
                matched_timestamp = matched_caption_info.get('timestamp')
                matched_time_span = matched_caption_info.get('time_span')
                matched_caption = matched_caption_info.get('caption')
                
                logger.info(f"Processing user query at {query_time}s (matched with caption at {matched_timestamp}, diff={min_time_diff:.2f}s): {query_content[:50]}...")
                
                # 构建frame_info用于检索
                frame_info_for_query = {
                ...
                try:
                    ...
                    # 使用用户问题作为检索查询
                    retrieved_video_context, retrieved_chunk_context = loop.run_until_complete(streaming_videorag_query(
                        query_content,  # 使用用户问题作为查询
                        matched_time_span,
                        "user_query",  # service_type
                        datasets_type, 
                        self.entities_vdb,
                        self.text_chunks,
                        self.chunks_vdb,
                    ...
                    # 调用process_frame_with_proactive_service来处理用户查询
                    # 这里我们可以使用检索到的上下文作为retrieved_memory
                    query_result = self.process_frame_with_proactive_service(
                        datasets_type=datasets_type,
                        frame_info=frame_info_for_query,
                        accumulated_captions=accumulated_captions,
                        max_captions_per_level=3,
                    ...
                    user_query_responses.append({
                        "query_time": query_time,
                        "user_question": query_content,
                        "frame_timestamp": matched_timestamp,
                        "time_span": matched_time_span,
                        "retrieved_context": retrieved_response,
                        "response": query_result.get('gemini_response') if query_result else None,
                        "time_diff": min_time_diff
                    })
                    
                    processed_query_indices.add(query_idx)
                    logger.info(f"Successfully processed user query {query_idx + 1}/{len(user_queries)}")
                    
                except Exception as e:
                    logger.error(f"Failed to process user query at {query_time}s: {e}")
                    user_query_responses.append({
                        "query_time": query_time,
                        "user_question": query_content,
                ...
                logger.warning(f"No matching caption found for user query at {query_time}s (min_diff={min_time_diff:.2f}s)")
                user_query_responses.append({
                    "query_time": query_time,
                    "user_question": query_content,
                    "frame_timestamp": None,
                    ...
                    "retrieved_context": None,
                    "response": None,
                    "error": "No matching caption found within time tolerance"
                })
        
        # 将用户查询响应添加到结果中
        base_result['user_query_responses'] = user_query_responses
        
        # 保存用户查询响应到文件
        query_responses_file = os.path.join(self.working_dir, "user_query_responses.json")
        try:
            with open(query_responses_file, 'w', encoding='utf-8') as f:
                json.dump(user_query_responses, f, ensure_ascii=False, indent=2)
            logger.info(f"Saved {len(user_query_responses)} user query responses to {query_responses_file}")
        except Exception as e:
            logger.error(f"Failed to save user_query_responses to {query_responses_file}: {e}")
