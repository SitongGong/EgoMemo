...
class VideoGraph:
    ...
    def process_frame_with_proactive_service(self, frame_info, accumulated_captions, max_captions_per_level=3, history_messages=None, retrieved_memory=None):
        ...
        try:
            ...
            #     # First try to find JSON array (list)
            #     json_array_start = response_text.find('[')
            #     json_array_end = response_text.rfind(']') + 1
            #     if json_array_start >= 0 and json_array_end > json_array_start:
            #         try:
            #             parsed_response = json.loads(response_text[json_array_start:json_array_end])
            #         except:
            #             pass
                
            #     # If array parsing failed, try to find JSON object
            #     if parsed_response is None:
            #         json_obj_start = response_text.find('{')
            #         json_obj_end = response_text.rfind('}') + 1
            #         if json_obj_start >= 0 and json_obj_end > json_obj_start:
            #             try:
            #                 parsed_response = json.loads(response_text[json_obj_start:json_obj_end])
            #             except:
            #                 pass
                
            #     # If both failed, try parsing the whole response
            #     if parsed_response is None:
            #         try:
    ...
    def streaming_graph_construction(self, data_path=None, anno_path=None, day=None, 
                       ...
                       max_new_tokens=512, enable_proactive_service=True, 
                        ...
                        if day_changed or is_gap or window_duration >= window_seconds:
                            ...
                            if current_window_frames:
                                ...
                                try:
                                    ...
                                    loop.run_until_complete(self.ainsert_streaming_caption({time_span: {"content": caption_text, "video_frames": current_window_frames, "type": "second"}}))    # 根据caption进行流式建图
                                    ...
                                    if enable_proactive_service:
                                        ...
                                        if not proactive_result or proactive_result.get('gemini_response') is None:
                                            ...
                                            if isinstance(gemini_response, list):
                                                ...
                                                # 尝试解析JSON字符串
                                                # import json
                                                parsed = parse_gemini_json_response(gemini_response)
                                            ...
                                            if needs_retrieval and retrieval_query:
                                                ...
                                                if proactive_result_with_memory and proactive_result_with_memory.get('gemini_response'):
                                                    ...
                                                    if isinstance(final_response, list):
                                                        ...
                                                        # import json
                                                        parsed = parse_gemini_json_response(final_response)
                                                        if isinstance(parsed, list):
                                                            service_list = parsed
                                                        elif isinstance(parsed, dict):
                                                            service_list = [parsed]
                                    ...
                                    if min_window_duration >= window_minutes * 60:
                                        ...
                                        if min_window_second_captions:
                                            ...
                                            user_prompt = f"{PROMPTS['min_caption_user_prompt']}\n\nCaptions:\n{caption_text}"
                                            ...
                                            try:
                                                ...
                                                if hour_window_duration >= window_hours * 3600:
                                                    ...
                                                    if hour_window_min_captions:
                                                        ...
                                                        user_prompt = f"{PROMPTS['hour_caption_user_prompt']}\n\nCaptions:\n{min_caption_text}"
            ...
            if current_window_frames:
                ...
                try:
                    caption = self.mllm_response(
                        self.video_llm,
                        self.processor,
                        PROMPTS["second_caption_user_prompt"],
