...
class VideoGraphSeparated:
    ...
    def streaming_graph_construction(
        ...
        Args:
            ...
            window_seconds: Window size in seconds for 10s caption generation (default: 30)
            gap_threshold_seconds: Gap threshold to start new window (default: 60)
            window_minutes: Window size in minutes for 10min caption generation (default: 5)
            ...
            frame_time_data: Optional list of dicts with time information for each frame
        ...
        def load_checkpoint_state():
            ...
            try:
                ...
                other_keys = ['current_window_frames', 'window_start_day', 
                             'window_start_timestamp', 'min_window_second_captions', 'min_window_start_day',
                             'min_window_start_timestamp', 'hour_window_min_captions', 'hour_window_start_day',
            ...
            for i, frame in enumerate(base64_frames):
                ...
                if window_start_timestamp is None:
                    ...
                    if last_frame_end_time is not None:
                        gap_duration = calculate_time_diff_seconds(start_time, last_frame_end_time, datasets_type)
                        ...
                        if current_window_frames:
                            ...
                            if time_span in second_captions:
                                ...
                                for frame_idx, content in caption["frames"].items():
                                    ...
                                    caption_dict[f"DAY{day_num}-{start_time_formatted}-{end_time_formatted}"] = content
                                second_captions[time_span] = {"dense_caption": caption_dict, "description": caption["caption"]}
                                ...
                                loop.run_until_complete(self.video_segments.upsert({time_span: {"content": json.dumps({"dense_caption": caption_dict, "description": caption["caption"]}), "video_frames": current_window_frames, "type": "second"}}))
                                ...
                                loop.run_until_complete(self.ainsert_streaming_caption({time_span: {"content": json.dumps({"dense_caption": caption_dict, "description": caption["caption"]}), "video_frames": current_window_frames, "type": "second"}}))
                                
                                # Add to accumulated_captions for proactive service (but don't call it here)
                                accumulated_captions['second_captions'].append({
                                    'time_span': time_span,
                                    'caption': json.dumps({"dense_caption": caption_dict, "description": caption["caption"]}),
                            ...
                            min_window_second_captions.append({
                                'time_span': time_span,
                                'caption': json.dumps({"dense_caption": caption_dict, "description": caption["caption"]}),  # 使用caption_dict而不是caption
                            ...
                            if min_window_duration >= window_minutes * 60:
                                ...
                                if min_window_second_captions:
                                    ...
                                    loop.run_until_complete(self.video_segments.upsert({min_time_span: {"content": min_caption, "sub_window_captions": [caption_dict["time_span"] for caption_dict in min_window_second_captions], "type": "minute"}}))
                                    loop.run_until_complete(self._save_video_segments())
                                    loop.run_until_complete(self.ainsert_streaming_caption({min_time_span: {"content": min_caption, "sub_window_captions": [caption_dict["time_span"] for caption_dict in min_window_second_captions], "type": "minute"}}))
        ...
        if current_window_frames:
            ...
            for retry_count in range(max_retries):
                ...
                try:
                    ...
                    if isinstance(caption, dict) and "frames" in caption and isinstance(caption["frames"], dict):
                        ...
                        logger.warning(f"Caption format invalid (attempt {retry_count + 1}/{max_retries}): missing 'frames' key or invalid structure")
                        if retry_count == max_retries - 1:
                            raise ValueError(f"Caption format invalid after {max_retries} attempts")
                except json.JSONDecodeError as e:
                    logger.warning(f"Failed to parse caption as JSON (attempt {retry_count + 1}/{max_retries}): {e}")
                    if retry_count == max_retries - 1:
                        raise ValueError(f"Failed to parse caption as JSON after {max_retries} attempts: {e}")
            ...
            for frame_idx, content in caption["frames"].items():
                start_time_formatted = format_timestamp_to_hhmmsscc(current_window_frame_data[int(frame_idx)]['start_time'])
                end_time_formatted = format_timestamp_to_hhmmsscc(current_window_frame_data[int(frame_idx)]['end_time'])
                caption_dict[f"DAY{start_day}-{start_time_formatted}-{end_time_formatted}"] = content
