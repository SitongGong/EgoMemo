...
# 每张输入帧 resize 到这个尺寸（短边/长边都 clamp 到该值），可通过环境变量覆盖。
# 设置为 0 或负数可禁用 resize（保持原图）。
# 默认 560x560 —— Qwen3-VL/2.5-VL 的 patch size 是 28，560 = 20 patches × 28；
# 单张图约 400 个 vision token，5 张图约 2k token，性价比高。
_DEFAULT_MLLM_IMAGE_SIZE = int(os.environ.get("MLLM_IMAGE_RESIZE", "560"))


def _maybe_resize_for_mllm(img: Image.Image, target: int = _DEFAULT_MLLM_IMAGE_SIZE) -> Image.Image:
    """把 VLM 输入帧按 target×target 做等比 resize 再居中裁剪，降低 vision token 数。

    保持 PIL Image 返回，供 qwen_vl_utils.process_vision_info 消费。
    target <= 0 时直接原样返回。
    """
    if target <= 0:
        return img
...
def _load_qwen_model(self, model_path: str, **kwargs):
    """Load Qwen VL model using transformers.
    ...
    # Determine which Qwen model class to use
    if "Qwen3.5" in model_path or "Qwen3_5" in model_path:
        from transformers import Qwen3_5ForConditionalGeneration
        self.video_llm = Qwen3_5ForConditionalGeneration.from_pretrained(
            model_path,
        ...
        try:
            ...
            self.video_llm = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                model_path,
                torch_dtype=torch.bfloat16,
                attn_implementation="flash_attention_2",
                device_map=device_map,
            ...
            # Fallback to Qwen2VL if Qwen2_5_VL is not available
            from transformers import Qwen2VLForConditionalGeneration
            self.video_llm = Qwen2VLForConditionalGeneration.from_pretrained(
                model_path,
                torch_dtype=torch.bfloat16,
                attn_implementation="flash_attention_2",
                device_map=device_map,
        ...
        self.video_llm = Qwen3VLForConditionalGeneration.from_pretrained(
            model_path,
            torch_dtype=torch.bfloat16,
            attn_implementation="flash_attention_2",
            device_map=device_map,
...
def mllm_response(video_llm, processor, user_prompt, system_prompt=None, base64_frames=None, max_new_tokens=512, has_image=True, temperature=None, top_p=None):
    ...
    if has_image and base64_frames is not None and len(base64_frames) > 0:
        ...
        for idx, frame_base64 in enumerate(base64_frames):
            try:
                ...
                # 下采样到固定尺寸（默认 560x560），降低 vision token 数
                frame_image = _maybe_resize_for_mllm(frame_image)
    ...
    # 移除 Qwen3.5 等模型的 <think>...</think> 推理块，只保留最终输出
    think_match = re.search(r'<think>.*?</think>\s*', result, re.DOTALL)
    if think_match:
