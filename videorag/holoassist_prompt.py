...
together covering a continuous time window of about 5-10 minutes.
...
- actions or operations that recur across multiple 30-second moments,
...
Together, these captions cover up to approximately one hour
...
You must focus on patterns that emerge ACROSS multiple 10-minute segments,
including:
...
(2) RECENT_10MIN_CAPTIONS:
    Several first-person summaries of recent ~10-minute windows.
(3) RECENT_1H_CAPTION:
    A first-person summary of the recent ~1-hour window (may be empty).
(4) RECENT_INTERACTION_HISTORY:
    Records of recent assistant-user interactions,
...
CRITICAL RULES:
- The trigger MUST be justified by evidence visible in CURRENT_30S_CAPTION.
- Longer-horizon captions may ONLY be used as supporting context
  (e.g., unfinished steps, repetition, pattern confirmation, or suppression).
- Do NOT trigger a service solely based on past captions.
...
If evidence is weak or ambiguous, prefer returning [].
...
1. Safety
Trigger if the CURRENT 30-second moment contains
any action, posture, or configuration that could immediately
cause bodily harm or an accident.

This includes (non-exhaustive):
- contact or close proximity to sharp tools, heat, electricity,
  moving machinery, unstable heavy objects;
- unsafe body positioning relative to hazards;
- loss of balance, slipping risk, or uncontrolled motion.

RULE:
If an action could plausibly injure the user RIGHT NOW,
classify as Safety, even if it also involves a tool.
...
- loose, misaligned, or unstable attachment;
...
- Tool Use = unsafe handling of a tool.
- Error-Recovery = a wrong step has already occurred.

2. Next-Step Guidance
Trigger if:
- a task or workflow is clearly underway;
- a step has been correctly completed;
- the CURRENT moment shows transition, pause, or preparation
  for the next stage, but the next expected step has not begun.

IMPORTANT:
If a step is DONE and the user is moving into a new phase,
you SHOULD actively consider Next-Step Guidance.

3. Resource Reminder
Trigger if the CURRENT moment shows the user
transitioning away while leaving an unresolved state behind.

Examples:
- tool still powered on;
- fastener not tightened;
- material not secured;
- workspace not reset or stabilized.
...
CRITICAL RULE — trigger_time_window resolution priority
	•	The trigger_time_window MUST be determined by first attempting to extract a precise time span from Second-level Captions (dense captions) if such timestamps are available.
	•	Second-level Captions refer to fine-grained, action-level annotations within the CURRENT_30S_CAPTION that explicitly specify a narrower time range (e.g., DAY# HH:MM:SS–HH:MM:SS) for a concrete action, error, or state.

Resolution priority (MANDATORY ORDER):
	1.	Second-level (dense) caption timestamps that directly correspond to the triggering action or state.
	2.	If multiple dense timestamps exist, select the one most causally aligned with the service trigger.
	3.	Only if no Second-level timestamp is available, fall back to the full 30-second window of CURRENT_30S_CAPTION.

Forbidden behavior (DO NOT DO):
	•	Defaulting to the full 30-second window when a dense timestamp exists.
	•	Selecting a timestamp that does not correspond to the triggering action.
	•	Averaging, expanding, or fabricating time spans.

Correct behavior:
	•	The trigger_time_window must be as tight and specific as possible, reflecting the exact moment the safety risk, tool misuse, error, or step completion occurs.
...
- Do NOT invent events not supported by captions.
...
If and ONLY if the decision requires memory
beyond the provided 10-minute or 1-hour captions,
...
Example 1 — Instant Safety (tool-related but classified as Safety):
...
[
  {
    ...
    "trigger_evidence": "I am holding a rotating power drill very close to my left hand while adjusting the bit.",
    "user_prompt": "Careful—your hand is very close to the rotating drill."
  }
]

------------------------------------------------------------

Example 2 — Tool Use (unsafe handling, no wrong step yet):
...
[
  {
    ...
    "trigger_evidence": "I am using a screwdriver at an unstable angle while the screw is not aligned.",
    "user_prompt": "The tool looks a bit unstable right now—adjusting it slightly might help."
  }
]

------------------------------------------------------------

Example 3 — Error-Recovery (wrong step already completed):
...
[
  {
    ...
    "trigger_evidence": "I have inserted the cable into the wrong port on the device.",
    "user_prompt": "That connection doesn't look correct—want to double-check the port?"
  }
]

------------------------------------------------------------

Example 4 — Next-Step Guidance (step completed, transitioning):
...
[
  {
    ...
    "confidence": "medium",
    "trigger_time_window": "DAY1 14:40:00-14:40:05",
    "trigger_evidence": "I finish tightening the last screw and then pause while looking at the remaining parts.",
    "user_prompt": "Looks like that part is done—ready to move on to the next step?"
  }
]
...
decide whether a proactive service
...
(2) RECENT_5MIN_CAPTIONS:
    Several first-person summaries of recent ~5-minute windows.
(3) RECENT_1H_CAPTION:
    A first-person summary of the recent ~1-hour window (may be empty).
(4) RETRIEVED_MEMORY_EVIDENCE:
    Retrieved past memory records returned from a previous memory_query.
    Each record may include a caption/summary, an approximate time window,
    and brief contextual notes.
(5) RECENT_INTERACTION_HISTORY:
    Recent assistant-user interaction records (what was triggered, when,
    and whether the user accepted/ignored/rejected).

All captions and memory records are produced by other modules and are your ONLY evidence.
...
Decide whether the user should receive any proactive service triggered by the
CURRENT 30-second moment, using RETRIEVED_MEMORY_EVIDENCE as supporting evidence.

CRITICAL RULES:
- The trigger MUST be justified by evidence in CURRENT_30S_CAPTION.
- Retrieved memory evidence may ONLY:
  • confirm relevance,
  • disambiguate object/step identity,
  • strengthen or weaken confidence,
  • confirm persistence/repetition,
  • or confirm whether a suspected issue was already resolved.
- Retrieved memory evidence MUST NOT create a new trigger by itself.
- Use RECENT_INTERACTION_HISTORY to avoid redundant interventions.
  If a similar service was triggered very recently and the situation has not
  meaningfully changed, DO NOT trigger it again.
- If uncertain after considering memory, prefer returning [].
...
IMPORTANT:
Although CURRENT_30S_CAPTION spans ~30 seconds, “Instant” services can be triggered
if ANY moment within this window shows an immediately dangerous or unsafe action/state.

A) Instant Proactive Services (time horizon: seconds; ≤ 10s; justify by current scene alone)

1) Safety
Trigger if the CURRENT 30-second window contains ANY action, posture, or configuration
that could plausibly cause bodily harm or an accident RIGHT NOW.

This includes (non-exhaustive):
- close proximity/contact with sharp tools, heat/flame, exposed electricity,
  moving machinery, unstable heavy objects;
- unsafe body positioning relative to hazards;
- loss of control, slipping risk, or uncontrolled motion.

RULE:
If an action could injure the user RIGHT NOW, classify as Safety even if it involves a tool.

2) Tool Use
Trigger if a tool is being handled, configured, or operated in an unsafe/unstable/improper way,
BUT WITHOUT evidence that a wrong procedural step has already been completed.

Examples:
- incorrect grip/orientation/angle;
- loose/misaligned/unstable attachment;
- missing guard/protection;
- tool left running when it should be powered off;
- unstable handling that could lead to damage or minor risk but not clearly an immediate injury.

CRITICAL DISTINCTION:
- Safety = immediate injury/accident risk NOW.
- Tool Use = unsafe/unstable tool handling/configuration, but not necessarily immediate injury,
  and not a confirmed wrong step already completed.

B) Short-Term Proactive Services (time horizon: ~10s-10min; same continuous session)

1) Error-Recovery
Trigger ONLY if the user has JUST completed a clearly incorrect procedural step
that must be corrected or rolled back to proceed.

Examples:
- wrong component assembled;
- wrong cable/port connected;
- incorrect order of operations already executed;
- wrong target/slot/config selected and applied.

CRITICAL DISTINCTION:
- Tool Use = unsafe handling/configuration while performing a step.
- Error-Recovery = the wrong step is already done (the system state is now wrong).

2) Next-Step Guidance
Trigger if:
- a task/workflow is clearly underway,
- a step has been correctly completed (or is clearly finished),
- the CURRENT moment shows a transition/pause/preparation,
  and the next expected step is not yet started or is missing/delayed.

IMPORTANT:
If a step is DONE and the user is moving into a new phase, you SHOULD actively consider
Next-Step Guidance.

3) Resource Reminder
Trigger if the CURRENT moment shows the user transitioning away while leaving an unresolved
or unstable state behind that should be closed/secured to avoid problems later.

Examples:
- tool still powered on / left running;
- fastener not tightened / part not secured;
- material not clamped/stabilized;
- workspace not reset (e.g., loose parts scattered) when they are moving on;
- protective cover/guard not replaced after use.
...
- Do NOT invent events not supported by CURRENT_30S_CAPTION and/or memory evidence.
- Do NOT output anything outside the required format.
- Multiple service objects are allowed but should be rare.

--------------------------------------------------------------------
How to Use Retrieved Memory Evidence (MANDATORY)
--------------------------------------------------------------------

1) Validation, not creation
Retrieved memory can ONLY validate/clarify/suppress a trigger that is already suggested
by CURRENT_30S_CAPTION.

2) Temporal alignment
- Prefer memory records close in time to the current moment (same session).
- If memory is far away or mismatched, treat it as weak support.

3) Conflict handling
If memory indicates the suspected issue is already resolved or contradicts the situation,
lower confidence or suppress the trigger.

4) Interaction suppression
If memory/interaction history shows a similar service was recently triggered and handled,
and nothing meaningfully changed, suppress the trigger.
...

{
  ...
  "service_sub_type":
    "Safety" | "Tool Use" | "Error-Recovery" | "Next-Step Guidance" | "Resource Reminder",
  ...
  "trigger_evidence":
    "A short factual statement grounded strictly in CURRENT_30S_CAPTION,
     optionally clarified/strengthened/suppressed by retrieved memory evidence.
     Do NOT include advice.",
...
Based on the given inputs (including retrieved memory evidence),
decide whether a proactive service should be triggered in the CURRENT 30-second moment
and output STRICTLY in the specified format.
"""
