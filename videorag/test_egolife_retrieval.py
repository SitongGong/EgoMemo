...
from .proassist_prompt import PROASSIST_PROMPTS
...
class VideoGraphSeparated:
    ...
    def streaming_graph_construction(
        ...
        for video_idx, video_path in enumerate(days_list):     # 读取视频和注释文件
            ...
            if datasets_type == "egolife":
                ...
                base64_frames_ = video_path["images"]      # num_frames, 3, H, W
                # 生成frame_timestamps和frame_time_ranges
                # 2fps意味着每0.5秒一帧，每帧有独立的时间戳
                frame_timestamps = []
                frame_time_ranges = []
                
                time_span_info = None
                base64_frames = []
                for i in range(len(base64_frames_)):
                    # 每帧有独立的时间戳：帧0是0.5秒，帧1是1.0秒，帧2是1.5秒，帧3是2.0秒，以此类推
                    timestamp_seconds = (i + 1) * 0.5  # 每帧独立时间戳
                    ...
                    # 生成时间范围：每帧有独立的时间范围
                    # 帧0（0.5秒）：[0, 1.0]秒，帧1（1.0秒）：[0.5, 1.5]秒，帧2（1.5秒）：[1.0, 2.0]秒，以此类推
                    range_start_seconds = i * 0.5
                    range_end_seconds = (i + 2) * 0.5
                    
                    # 将tensor转换为PIL Image，然后编码为base64字符串
                    pil_images = tensor_to_pil_images(base64_frames_[i:i+1])
                    if pil_images:
                        ...
                        if current_window_frames:
                            ...
                            if time_span in second_captions:
                                ...
                                if datasets_type == "egolife":
                                    ...
                                    caption_prompt = PROASSIST_PROMPTS["simple_second_caption_system_prompt"]
                                else:
                                ...
                                for retry_count in range(max_retries):
                                    try:
                                        ...
                                        for frame_idx, content in caption["frames"].items():
                                            ...
                                            # 对于proassist数据集，每帧有独立的时间戳和时间范围，不需要添加帧编号
    ...
    def process_frame_with_proactive_service(self, frame_info, accumulated_captions, datasets_type, max_captions_per_level=3, history_messages=None, retrieved_memory=None, formatted_system_info=None):
        ...
        if history_messages and isinstance(history_messages, str) and history_messages.strip() and datasets_type != "proassist":
            ...
            proactive_prompt = proactive_prompt + "\n\n" + "=== Recent Assistant Response History ===\n" + history_messages
        ...
        for idx, second_caption_info in enumerate(second_captions_list):
            ...
            if parsed_response:
                ...
                if isinstance(finalized_services, dict):
                    ...
                    if decision == 'finalize' and isinstance(services, list):
            ...
            if isinstance(episodic_longterm_probe, dict) and episodic_longterm_probe:
                ...
                if second_captions_list:
                    ...
                    # 提取起始时间：第一个caption的开始时间
                    # if first_time_span:
                    #     first_converted = convert_timestamp_format(first_time_span)
                    #     first_parts = first_converted.split('-')
                    #     if len(first_parts) >= 2:
                    #         video_start_time = f"{first_parts[0]}-{first_parts[1]}"
                    
                    # 提取结束时间：最后一个caption的结束时间
                    # if last_time_span:
                    #     last_converted = convert_timestamp_format(last_time_span)
                    #     last_parts = last_converted.split('-')
                    #     if len(last_parts) >= 3:
                    #         video_end_time = f"{last_parts[0]}-{last_parts[2]}"
