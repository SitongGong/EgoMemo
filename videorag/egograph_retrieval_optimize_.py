...
from .eyewo_prompt_ import EYEWO_PROMPTS, OUTPUT_FORMAT
from .offline_prompt import OFFLINE_PROMPTS
from .ovobench_prompt import OVOBENCH_PROMPTS
...
from .video_processing import sample_frames_by_interval, extract_eyewo_video_frames, sample_frames_qaego4d
...
def format_system_info(system_info_list, task_goal, task_knowledge):
    """
    ...
    formatted_text += f"Task Goal: {task_goal} \n\n Task Knowledge: {task_knowledge}\n\n"
...
MODEL_MAP = {
    "qwenvl_3_8b_instruct": os.environ.get("QWEN3VL_8B_PATH", "./pretrained_weights/Qwen3-VL-8B-Instruct"),  # "Qwen/Qwen3-VL-8B-Instruct"
    "qwenvl_3_4b_instruct": "Qwen/Qwen3-VL-4B-Instruct",
    "qwenvl_3_5_2b": os.environ.get("QWEN35_2B_PATH", "./pretrained_weights/Qwen3.5-2B"),
    "qwenvl_3_5_4b": os.environ.get("QWEN35_4B_PATH", "./pretrained_weights/Qwen3.5-4B"),
    "qwenvl_3_5_9b": os.environ.get("QWEN35_9B_PATH", "./pretrained_weights/Qwen3.5-9B"),
    ...
    "minicpm_4_5_o": "openbmb/MiniCPM-o-4_5",
}
...
class VideoGraphSeparated:
    ...
    video_embedding_gpu: int = 0
    
    # query
    retrieval_topk_chunks: int = 3
    ...
    entity_summary_to_max_tokens: int = 2048
    ...
    def __post_init__(self):
        ...
        self.video_segments = self.key_string_value_json_storage_cls(    # 一般正常数量
            namespace="video_segments", global_config=asdict(self)
        )

        self.text_chunks = self.key_string_value_json_storage_cls(     # 一般会多出一个text chunk
    ...
    def load_caption_model(self, model_name: str, device_map: Optional[Union[str, Dict]] = None, **api_kwargs):
        ...
        elif "vllm" in model_name_lower:
            # 使用本地 vLLM 服务（OpenAI 兼容接口）加载 Qwen3-VL-8B
            # 通过 HTTP 调用本机/远程 vLLM serve 端点，无需在本进程加载模型权重。
            # 启动服务: bash scripts/launch_vllm_qwen3vl_v023.sh (默认 GPU0:8000)
            from .llm.qwen3vl_vllm import mllm_response as qwen_vllm_mllm_response_func

            # base_url 优先级: 显式 api_kwargs > 环境变量 VLLM_BASE_URL > 默认本机 8000
            vllm_base_url = (
                api_kwargs.get("base_url")
                or api_kwargs.get("vllm_base_url")
                or os.environ.get("VLLM_BASE_URL")
                or "http://localhost:8000/v1"
            )
            vllm_model_name = (
                api_kwargs.get("model_name")
                or api_kwargs.get("vllm_model_name")
                or "Qwen3-VL-8B-Instruct"
            )

            # 创建包装函数以传递 vLLM 端点参数（签名与其他后端保持一致）
            def qwen_vllm_mllm_response(video_llm, processor, user_prompt, system_prompt=None,
                                        base64_frames=None, max_new_tokens=2048, has_image=True,
                                        temperature=None, top_p=None):
                return qwen_vllm_mllm_response_func(
                    video_llm, processor, user_prompt, system_prompt, base64_frames,
                    max_new_tokens, has_image, temperature, top_p,
                    base_url=vllm_base_url,
                    api_key=api_kwargs.get("api_key", "EMPTY"),
                    model_name=vllm_model_name,
                )

            self.mllm_response = qwen_vllm_mllm_response

            # 设置占位符对象（vLLM 走 HTTP，不需要在本进程加载模型实例）
            self.caption_model = None
            self.caption_processor = None
            self.processor = None
            self.video_llm = None
            self.image_processor = None
            logger.info(f"Loaded Qwen vLLM caption backend: base_url={vllm_base_url} model={vllm_model_name}")
            return
        ...
        # 确定device_map和max_memory：如果指定了device_map参数，使用它；否则检查CUDA_VISIBLE_DEVICES环境变量
        max_memory = None
        if device_map is None:
            ...
            if cuda_visible_devices:
                ...
                device_map = {"": 0}
                # 限制CPU内存使用，强制模型只加载到GPU
                max_memory = {0: "140GB", "cpu": "10GB"}
                logger.info(f"CUDA_VISIBLE_DEVICES={cuda_visible_devices} detected, using device_map={{'': 0}}, max_memory={max_memory}")
            else:
                # 否则使用auto自动分配
                device_map = "auto"
                ...
                max_memory["cpu"] = "10GB"
                logger.info(f"No CUDA_VISIBLE_DEVICES set, using device_map='auto', max_memory={max_memory}")
        else:
        ...
        if "minicpm" in model_name.lower() or "MiniCPM" in model_path:
            ...
            if "Qwen3.5" in model_path or "Qwen3_5" in model_path:
                ...
                video_llm = Qwen3_5ForConditionalGeneration.from_pretrained(
                    model_path,
                    **load_kwargs
                )
    ...
    def streaming_graph_construction(
        ...
        question_with_option=None,
        user_query_list=None, 
        ego4d_video_time_span=None, 
    ):
        """
        ...
        #     anno_list = []
            ...
            days_list = [data_path]
            anno_list = []
        elif datasets_type == "egotaskqa" or datasets_type == "qaego4d":
            days_list = [data_path]
            anno_list = []
        elif datasets_type == "ovobench":
            days_list = [data_path]
            anno_list = []
        else:
        ...
        for video_idx, video_path in enumerate(days_list):     # 读取视频和注释文件
            ...
            try:
                ...
                elif datasets_type == "holoassist" or datasets_type == "captioncook4d" or datasets_type == "egoschema":
                    # 每interval_seconds提取一帧，同时获取各帧的时间范围
                    video_name = os.path.basename(video_path).split(".")[0]
                    time_span_info = None
                    base64_frames, frame_timestamps, frame_time_ranges = sample_frames_by_interval(
                ...
                elif datasets_type == "qaego4d":
                    video_name = os.path.basename(video_path).split(".")[0]
                    time_span_info = None
                    base64_frames, frame_timestamps, frame_time_ranges = sample_frames_qaego4d(
                        video_path,
                        ego4d_video_time_span, 
                        interval_seconds=interval_seconds,
                        output_format='base64',
                    )
                            ...
                            if current_window_frames:
                                ...
                                # 检查该time_span的caption是否已经生成（避免重复生成）
                                    ...
                                    if datasets_type == "egolife":
                                        ...
                                        caption_prompt = EYEWO_PROMPTS["simple_second_caption_system_prompt"].format(questions=", ".join(user_query_list), output_format=OUTPUT_FORMAT)
                                    elif datasets_type == "egoschema":
                                        caption_prompt = EGOSCHEMA_PROMPTS["caption_system_prompt_with_query"].format(question=question_with_option, output_format=OUTPUT_FORMAT) 
                                    elif datasets_type == "qaego4d" or datasets_type == "egotaskqa":
                                        caption_prompt = OFFLINE_PROMPTS["caption_system_prompt_with_query"].format(question=question_with_option, output_format=OUTPUT_FORMAT) 
                                    elif datasets_type == "ovobench":
                                        caption_prompt = OVOBENCH_PROMPTS["simple_second_caption_system_prompt"].format(questions=question_with_option, output_format=OUTPUT_FORMAT) 
                                    else:
                                    ...
                                    if gap_between_second_captions >= gap_threshold_seconds:
                                        ...
                                        if min_window_second_captions:
                                            ...
                                            if datasets_type == "egolife":
                                                ...
                                                user_prompt = f"{OFFLINE_PROMPTS['min_caption_system_prompt']}\n\nCaptions:\n{caption_text}"
                                            elif datasets_type == "ovobench":
                                                user_prompt = f"{OVOBENCH_PROMPTS['min_caption_system_prompt']}\n\nCaptions:\n{caption_text}"
                                            else:
                                            ...
                                            if gap_between_min_captions >= gap_threshold_seconds:
                                                ...
                                                if hour_window_min_captions:
                                                    ...
                                                    if datasets_type == "egolife":
                                                        ...
                                                        user_prompt = f"{OFFLINE_PROMPTS['hour_caption_system_prompt']}\n\nCaptions:\n{min_caption_text}"
                                                    elif datasets_type == "ovobench":
                                                        user_prompt = f"{OVOBENCH_PROMPTS['hour_caption_system_prompt']}\n\nCaptions:\n{min_caption_text}"
                                                    elif datasets_type == "egoextra" or datasets_type == "captioncook4d":
                                                        user_prompt = f"{EGOEXTRA_PROMPTS['hour_caption_system_prompt']}\n\nCaptions:\n{min_caption_text}"
                                                    else:
                                                        user_prompt = f"{HOLOASSIST_PROMPTS['hour_caption_system_prompt']}\n\nCaptions:\n{min_caption_text}"
        ...
        if current_window_frames:
            ...
            # 若该 time_span 已在 second_captions 中（如从 checkpoint 恢复），则不再重新生成、不重复 upsert、不重复 append，避免每次重跑都重复写入 text_chunks 和 min_window
            if time_span in second_captions:
                caption_dict = second_captions[time_span]
                logger.info(f"Final 10s window {time_span} already in second_captions (resume), skip regenerate and duplicate appends")
            else:
                # Generate 10s caption using Qwen model with retry mechanism
                if datasets_type == "egolife":
                    caption_prompt = PROMPTS["simple_second_caption_system_prompt"]
                    ...
                    caption_prompt = EYEWO_PROMPTS["simple_second_caption_system_prompt"].format(questions=", ".join(user_query_list), output_format=OUTPUT_FORMAT)   # task_types=", ".join(task_type)
                elif datasets_type == "egoschema":
                    caption_prompt = EGOSCHEMA_PROMPTS["caption_system_prompt_with_query"].format(question=question_with_option, output_format=OUTPUT_FORMAT)
                elif datasets_type == "qaego4d" or datasets_type == "egotaskqa":
                    caption_prompt = OFFLINE_PROMPTS["caption_system_prompt_with_query"].format(question=question_with_option, output_format=OUTPUT_FORMAT)
        ...
        if min_window_second_captions:
            ...
            if datasets_type == "egoschema" or datasets_type == "qaego4d" or datasets_type == "egotaskqa":
            ...
            if min_window_duration >= min_threshold_seconds:
                ...
                # 若该 min_time_span 已在 min_captions 中（如从 checkpoint 恢复），则不再重新生成、不重复 upsert、不重复 append
                if min_time_span in min_captions:
                    min_caption = min_captions[min_time_span]
                    logger.info(f"Final 10min window {min_time_span} already in min_captions (resume), skip regenerate and duplicate appends")
                else:
                    caption_text = "\n".join([f"[{item['time_span']}]: {item['caption']}" for item in min_window_second_captions])
                    if datasets_type == "egolife":
                        user_prompt = f"{PROMPTS['min_caption_system_prompt']}\n\nCaptions:\n{caption_text}"
                ...
                # Add to 1-hour window（无论是否 resume，都要保证 hour_window_min_captions 有该项以便后续 1h 聚合；若 resume 则列表已从 checkpoint 恢复）
                if hour_window_start_timestamp is None:
                    hour_window_start_timestamp = min_window_start_timestamp
                    hour_window_start_day = min_start_day
                
                # 仅当该项尚未在 hour_window_min_captions 中时才 append，避免 resume 时重复
                existing_min_time_spans = {item['time_span'] for item in hour_window_min_captions}
                if min_time_span not in existing_min_time_spans:
                    final_timestamp = min_window_second_captions[-1]['end_timestamp'] if min_window_second_captions else min_window_start_timestamp
                    hour_window_min_captions.append({
                        'time_span': min_time_span,
                        'caption': min_caption,
        ...
        if hour_window_min_captions:
            ...
            if hour_window_duration >= hour_threshold_seconds:
                ...
                # 若该 hour_time_span 已在 hour_captions 中（如从 checkpoint 恢复），则不再重新生成、不重复 upsert、不重复 append
                if hour_time_span in hour_captions:
                    logger.info(f"Final 1h window {hour_time_span} already in hour_captions (resume), skip regenerate and duplicate appends")
                else:
                    min_caption_text = "\n".join([f"[{item['time_span']}]: {item['caption']}" for item in hour_window_min_captions])
                    if datasets_type == "egolife":
                        user_prompt = f"{PROMPTS['hour_caption_system_prompt']}\n\nCaptions:\n{min_caption_text}"
    ...
    async def ainsert_streaming_caption(self, new_video_segments, datasets_type: str = None, task_topic: str = None, task_types: list[str] = None):
        ...
        inserting_chunks = {compute_mdhash_id(caption_dict["time_span"][0], prefix="chunk-"): caption_dict}   # caption_dict["content"]
    ...
    def process_frame_with_proactive_service(self, frame_info, accumulated_captions, datasets_type, max_captions_per_level=3, history_messages=None, retrieved_memory=None, formatted_system_info=None, pre_retrieval_probe=None, if_multiscale=True):
        """
        Args:
        ...
        if current_caption_parts and if_multiscale:
        ...
        # 解析 response：可能是纯 JSON 字符串，也可能是 ```json ... ``` 格式
        parsed_response = None
        if response and isinstance(response, str):
            resp_stripped = response.strip()
            # 先尝试直接解析
            try:
                parsed_response = json.loads(resp_stripped)
            except json.JSONDecodeError:
                # 尝试提取 ```json ... ``` 代码块
                json_match = re.search(r'```(?:json)?\s*\n(.*?)\n?```', resp_stripped, re.DOTALL | re.IGNORECASE)
                if json_match:
                    try:
                        parsed_response = json.loads(json_match.group(1).strip())
                    except json.JSONDecodeError:
                        parsed_response = resp_stripped
                else:
                    parsed_response = resp_stripped
        return {
    ...
    def process_proactive_service(
        ...
        proactive_checkpoint_file = os.path.join(self.working_dir, f"{args.save_name}.json")
        ...
        for idx, second_caption_info in enumerate(second_captions_list):
            ...
            history_text = format_proactive_history(proactive_service_history[-5:])
            ...
            if needs_retrieval and retrieval_query:
                ...
                if not isinstance(first_response, str):
                    first_response = json.dumps(first_response, ensure_ascii=False)
                ...
                if first_prompt and first_response:
                    ...
                    if datasets_type == "egolife":
                        ...
                        memory_prompt = HOLOASSIST_PROMPTS["proactive_service_prompt_with_memory_simple"]
                ...
                if final_response:
                    # 解析第二次 LLM 的响应并更新 service_list
                    parsed_final = None
                    if isinstance(final_response, (dict, list)):
                        parsed_final = final_response
                    elif isinstance(final_response, str):
                        resp_stripped = final_response.strip()
                        try:
                            parsed_final = json.loads(resp_stripped)
                        except json.JSONDecodeError:
                            json_match = re.search(r'```(?:json)?\s*\n(.*?)\n?```', resp_stripped, re.DOTALL | re.IGNORECASE)
                            if json_match:
                                try:
                                    parsed_final = json.loads(json_match.group(1).strip())
                                except json.JSONDecodeError:
                                    parsed_final = resp_stripped

                    proactive_result['gemini_response'] = parsed_final
                    proactive_result['retrieved_memory'] = retrieved_response

                    # 从第二次响应中重新提取 service_list
                    # LLM 输出格式只有两种：
                    #   1. list: [{service_main_type, ...}, ...]
                    #   2. dict: {"decision": "suppressed", "reason": "..."}
                    if isinstance(parsed_final, list):
                        service_list = parsed_final
                    elif isinstance(parsed_final, dict):
                        if parsed_final.get('decision') == 'suppressed':
                            service_list = []
                            proactive_result['suppressed_reason'] = parsed_final.get('reason', '')
                        else:
                            # 单个 service dict，包装成 list
                            service_list = [parsed_final]
            ...
            if service_list:
                ...
                for service in service_list:
                    if isinstance(service, dict):
                        tw = service.get('trigger_time_window', '')
                        if tw and not tw.startswith('DAY') and '-' in tw:
                            p = tw.split('-')
                            tw = f"DAY{p[0]}-{'-'.join(p[1:])}"
        ...
        proactive_responses_file = os.path.join(self.working_dir, f"{args.response_name}.json")
        try:
    ...
    def streaming_retrieval_with_user_queries(
        ...
        # 加载proactive service检查点（用于断点续传）
        proactive_checkpoint_file = os.path.join(self.working_dir, f"proactive_service_checkpoint_{index}.json")
        processed_time_spans = set()
        if os.path.exists(proactive_checkpoint_file):
            try:
                with open(proactive_checkpoint_file, 'r', encoding='utf-8') as f:
        ...
        for idx, second_caption_info in enumerate(second_captions_list):
            ...
            # 将对话历史格式化为文本 prompt
            formatted_system_info = format_system_info(system_info, task_goal, task_knowledge)
        ...
        # 保存proactive_responses到文件
        proactive_responses_file = os.path.join(self.working_dir, f"proactive_responses_{index}.json")
        try:
            with open(proactive_responses_file, 'w', encoding='utf-8') as f:
                json.dump(proactive_responses, f, ensure_ascii=False, indent=2)
            logger.info(f"Saved {len(proactive_responses)} proactive responses to {proactive_responses_file}")
    ...
    def process_egolife_proactive_service(
        ...
        for idx, second_caption_info in enumerate(second_captions_list):
            ...
            # 第一次调用：使用Gemini判断是否需要主动服务
            # 传入之前的检索信息以抑制频繁检索
            history_text = format_proactive_history(proactive_service_history[-10:])
            proactive_result = self.process_frame_with_proactive_service(
                datasets_type=datasets_type,
                frame_info=frame_info_for_proactive,
                accumulated_captions=accumulated_captions,
                max_captions_per_level=3,
                history_messages=None, # history_text,
                retrieved_memory=None,
            ...
            if needs_retrieval and retrieval_query and time_key:
                ...
                if first_prompt and first_response:
                    ...
                    # 解析 final_response：可能是纯 JSON 字符串，也可能是 ```json ... ``` 格式
                    parsed_final = None
                    ...
                    # 更新proactive_result
                    proactive_result['gemini_response'] = parsed_final  # 更新为带记忆的响应
