...
class VideoRAG:
    ...
    def load_caption_model(self, debug=False):
        # caption model
        if not debug:
            self.caption_model = AutoModel.from_pretrained('/root/githubs/VideoRAG/MiniCPM-V-2_6-int4', trust_remote_code=True)
            self.caption_tokenizer = AutoTokenizer.from_pretrained('/root/githubs/VideoRAG/MiniCPM-V-2_6-int4', trust_remote_code=True)
    ...
    def insert_video(self, video_path_list=None):
        ...
        for video_path in video_path_list:
            ...
            # Step4: insert video segments information     把对每个30s clip生成的caption，时间跨度以及transcription进行整理
            ...
            loop.run_until_complete(self.video_segments.upsert(
                {video_name: segments_information}   # 存储一段视频对应的所有caption信息
            ))
    ...
    async def ainsert(self, new_video_segment):
        ...
        try:
            ...
            await self.text_chunks.upsert(inserting_chunks)    # 所以模型一共有两个地方包含video_segment_id，一个是在text chunks，还有一个是在video feature中
        finally:
