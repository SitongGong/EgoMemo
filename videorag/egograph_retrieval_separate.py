...
class VideoGraphSeparated:
    ...
    def load_captions_from_checkpoint(self, checkpoint_path: str = None) -> dict:
        """
        从 checkpoint 文件加载已生成的 captions 数据。

        Args:
            checkpoint_path: checkpoint 文件路径，默认为 working_dir/streaming_checkpoint.json

        Returns:
            dict: 包含 captions 数据的字典
        """
        if checkpoint_path is None:
            checkpoint_path = os.path.join(self.working_dir, "streaming_checkpoint.json")
        ...
        result = {
            'accumulated_captions': checkpoint_data.get('accumulated_captions', {}),
            'captions_dict': checkpoint_data.get('captions_dict', {}),
            'window_states': checkpoint_data.get('window_states', {}),
            'processed_time_spans': set(checkpoint_data.get('processed_time_spans', []))
        }

        # 尝试从数据库加载 video_segments
        try:
            if hasattr(self.video_segments, '_data'):
                result['video_segments'] = dict(self.video_segments._data)
            else:
                result['video_segments'] = {}
        except Exception as e:
            logger.warning(f"Failed to load video_segments from database: {e}")
            result['video_segments'] = {}

        logger.info(f"Loaded captions from checkpoint: {len(result['accumulated_captions'].get('second_captions', []))} second captions, "
                   f"{len(result['accumulated_captions'].get('min_captions', []))} min captions")

        return result

    def extract_entities_from_checkpoint(
        self,
        checkpoint_path: str = None,
        datasets_type: str = "egolife",
        task_topic: str = None,
        task_types: list = None,
        batch_size: int = 10
    ) -> dict:
        """
        从已有的 checkpoint 文件中提取实体和关系。
        适用于已经生成了 caption 但还未提取实体的情况。
        ...
        # 加载 checkpoint 数据
        captions_data = self.load_captions_from_checkpoint(checkpoint_path)
        accumulated_captions = captions_data['accumulated_captions']
        processed_time_spans = captions_data['processed_time_spans']

        # 获取所有 second captions（只有 second 级别需要提取实体）
        second_captions = accumulated_captions.get('second_captions', [])
        if not second_captions:
            logger.warning("No second captions found in checkpoint")
        ...
        for cap in second_captions:
            time_span = cap.get('time_span', '')
            if time_span and time_span not in processed_time_spans:
                pending_captions.append(cap)

        if not pending_captions:
            logger.info("All captions have been processed for entity extraction")
            return {'extracted_count': 0, 'skipped_count': len(second_captions)}

        logger.info(f"[Entity Extraction] Processing {len(pending_captions)} pending captions")

        # 批量提取实体
        extracted_count = 0

        for i in range(0, len(pending_captions), batch_size):
            batch = pending_captions[i:i + batch_size]
            logger.info(f"Processing batch {i // batch_size + 1}/{(len(pending_captions) + batch_size - 1) // batch_size}")

            for cap in batch:
                time_span = cap.get('time_span', '')
                caption_content = cap.get('caption', '')

                if isinstance(caption_content, dict):
                    caption_text = caption_content.get('caption', str(caption_content))
                else:
                    caption_text = str(caption_content)

                # 构建 chunk
                ENCODER = tiktoken.encoding_for_model("gpt-4o")
                tokens = ENCODER.encode(caption_text)
                ...
                chunk_key = compute_mdhash_id(caption_dict["time_span"][0], prefix="chunk-")
                inserting_chunks = {chunk_key: caption_dict}

                # 检查是否已存在
                _add_chunk_keys = loop.run_until_complete(
                ...
                try:
                    ...
                    maybe_new_kg, _, _ = loop.run_until_complete(
                        streaming_extract_entities(
                            chunk_key,
                            caption_dict,
                            self.chunk_entity_relation_graph,
                    ...
                    # 保存到 naive RAG
                    if self.enable_naive_rag:
                        loop.run_until_complete(self.chunks_vdb.upsert(inserting_chunks))

                    loop.run_until_complete(self.text_chunks.upsert(inserting_chunks))
                    loop.run_until_complete(self._insert_done())

                except Exception as e:
                    logger.error(f"Failed to extract entities for {time_span}: {e}")
                    continue

        logger.info(f"[Entity Extraction] Completed: extracted {extracted_count} entities")

        return {
            'extracted_count': extracted_count,
            'skipped_count': len(second_captions) - len(pending_captions),
            'total_count': len(second_captions)
        }

    def encode_visual_embeddings_from_checkpoint(
        self,
        checkpoint_path: str = None,
        video_path: str = None,
        encode_mode: str = "joint"
    ) -> dict:
        """
        从已有的 checkpoint 文件中编码 visual embeddings。
        适用于已经生成了 caption 但还未编码 visual embedding 的情况。

        Args:
            checkpoint_path: checkpoint 文件路径
            video_path: 视频文件路径（用于提取帧）
            encode_mode: 编码模式，"joint" 或 "separate"
        ...
        # 从 video_segments 数据库获取所有 second 类型的 segments
        try:
            if hasattr(self.video_segments, '_data'):
                all_segments = dict(self.video_segments._data)
            else:
                logger.error("Cannot access video_segments._data")
                return {'encoded_count': 0, 'error': 'Cannot access video_segments'}
        except Exception as e:
            logger.error(f"Failed to load video_segments: {e}")
            return {'encoded_count': 0, 'error': str(e)}

        # 筛选出需要编码的 segments（second 类型且未处理）
        pending_segments = []
        for time_span, segment_data in all_segments.items():
            if segment_data.get('type') == 'second' and time_span not in processed_time_spans:
                video_frames = segment_data.get('video_frames', [])
                if video_frames:
                    pending_segments.append((time_span, video_frames))

        if not pending_segments:
            logger.info("All video segments have been encoded")
            return {'encoded_count': 0, 'skipped_count': len(all_segments)}

        logger.info(f"[Visual Embedding] Processing {len(pending_segments)} pending segments")
        ...
        try:
            ...
            logger.info(f"[Visual Embedding] Completed: encoded {len(pending_segments)} segments")

            return {
                'encoded_count': len(pending_segments),
                'skipped_count': len(all_segments) - len(pending_segments),
                'total_count': len(all_segments)
            }

        except Exception as e:
            logger.error(f"Failed to encode visual embeddings: {e}")
            return {'encoded_count': 0, 'error': str(e)}

    def process_from_existing_captions(
        self,
        checkpoint_path: str = None,
        video_path: str = None,
        datasets_type: str = "egolife",
        extract_entities: bool = True,
        encode_visual: bool = True,
        task_topic: str = None,
        task_types: list = None
    ) -> dict:
        """
        从已有的 caption 文件进行后续处理（实体提取和/或 visual embedding 编码）。
        这是一个便捷函数，结合了 extract_entities_from_checkpoint 和 encode_visual_embeddings_from_checkpoint。

        使用示例:
            # 仅提取实体
            videorag.process_from_existing_captions(extract_entities=True, encode_visual=False)

            # 仅编码 visual embedding
            videorag.process_from_existing_captions(extract_entities=False, encode_visual=True)

            # 两者都处理
            videorag.process_from_existing_captions(extract_entities=True, encode_visual=True)
        ...
        Args:
            ...
            encode_visual: 是否编码 visual embedding
            task_topic: 任务主题
            task_types: 任务类型列表
        ...
        if extract_entities:
            logger.info("=" * 60)
            logger.info("Step 1: Extracting entities from captions...")
            logger.info("=" * 60)
            entity_result = self.extract_entities_from_checkpoint(
        ...
        if encode_visual:
            logger.info("=" * 60)
            logger.info("Step 2: Encoding visual embeddings...")
            logger.info("=" * 60)
            visual_result = self.encode_visual_embeddings_from_checkpoint(
                checkpoint_path=checkpoint_path,
                video_path=video_path
            )
            results['visual_embedding'] = visual_result
