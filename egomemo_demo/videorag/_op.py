...
async def _merge_nodes_then_upsert(
    ...
    already_node = await knowledge_graph_inst.get_node(entity_name)    # 将更新的节点信息加入到已有节点中
    ...
    description = await _handle_entity_relation_summary(      # 如果description过长，则进行总结
    ...
    await knowledge_graph_inst.upsert_node(     # 将节点信息加入知识图谱中
        entity_name,
        node_data=node_data,
    )
...
async def _merge_edges_then_upsert(
    ...
    already_description = []
    already_order = []
    if await knowledge_graph_inst.has_edge(src_id, tgt_id):    # 如果已存在关系，则将权重和description加入到已有关系中
    ...
    description = await _handle_entity_relation_summary(     # 压缩太长的description
        (src_id, tgt_id), description, global_config
    )
    await knowledge_graph_inst.upsert_edge(     # 将关系信息加入知识图谱中
        src_id,
        tgt_id,
        edge_data=dict(
...
async def _find_most_related_segments_from_entities(
    ...
    edges = await asyncio.gather(      # 得到每个节点对应的边
        *[knowledge_graph_inst.get_node_edges(dp["entity_name"]) for dp in node_datas]
    )
    all_one_hop_nodes = set()      # 得到所有边对应的one-hop节点集合
    for this_edges in edges:
        if not this_edges:
            continue
        all_one_hop_nodes.update([e[1] for e in this_edges])
    all_one_hop_nodes = list(all_one_hop_nodes)
    all_one_hop_nodes_data = await asyncio.gather(      # 得到所有one-hop节点对应的节点信息
...
async def videorag_query(
    ...
    for s_id in retrieved_segments:
        ...
        rough_captions[s_id] = video_segments._data[video_name][index]["content"]     # 这里是保存了同一个视频的多个clip的caption内容
    results = await asyncio.gather(
