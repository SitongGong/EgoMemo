...
from .proassist_prompt_ import PROASSIST_PROMPTS
from .eyewo_prompt_ import EYEWO_PROMPTS
from .egoschema_prompt_ import EGOSCHEMA_PROMPTS
...
from ._videoutil import (
    ...
    retrieved_segment_caption_qwen3_api, 
)
...
def get_prompts(global_config: dict = None, datasets_type: str = None):
    ...
    if datasets_type and datasets_type.lower() in ["holoassist", "egoextra", "ego_extra", "captioncook4d"]:
        return HOLOASSIST_PROMPTS
    elif datasets_type and datasets_type.lower() in ["proassist", "pro_assist"]:
        return PROASSIST_PROMPTS
    elif datasets_type and datasets_type.lower() in ["eyewo", "eye_wo"]:
        return EYEWO_PROMPTS
    elif datasets_type and datasets_type.lower() in ["egoschema", "ego_schema"]:
        return EGOSCHEMA_PROMPTS
    elif datasets_type == "egolife":
        return PROMPTS
    elif datasets_type == "egotaskqa" or datasets_type == "qaego4d":
        return OFFLINE_PROMPTS
    elif datasets_type == "ovobench":
        return OVOBENCH_PROMPTS
    else:
        raise ValueError(f"Invalid datasets_type: {datasets_type}")
...
async def _merge_nodes_then_upsert(
    ...
    description = await _handle_entity_relation_summary(
        entity_name, description, global_config, datasets_type
    )
    node_data = dict(
        entity_type=entity_type,
...
async def _merge_edges_then_upsert(
    ...
    description = await _handle_entity_relation_summary(
        (src_id, tgt_id), description, global_config, datasets_type
    )
...
async def streaming_extract_entities(
    ...
    all_entities_data = await asyncio.gather(
        *[
            _merge_nodes_then_upsert(k, v, knowledge_graph_inst, global_config, datasets_type)
            for k, v in maybe_nodes.items()
        ]
    )
    all_edges_data = await asyncio.gather(
        *[
            _merge_edges_then_upsert(k[0], k[1], v, knowledge_graph_inst, global_config, datasets_type)
...
async def batch_extract_entities(
    ...
    """
    批量并行提取多个chunks的实体和关系，使用asyncio.gather实现并行化。
    这是 streaming_extract_entities 的批量版本，用于在最后统一处理所有captions。
    
    Args:
        chunks: 要处理的chunks字典，key为chunk_key，value为TextChunkSchema
        knowledge_graph_inst: 知识图谱存储实例（全局图结构）
        entity_vdb: 实体向量数据库
        global_config: 全局配置
        datasets_type: 数据集类型
        task_topic: 任务主题（用于proassist数据集）
        task_types: 任务类型列表（用于eyewo数据集）
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        #         node_data for node_data in node_data_list
        #         if not (
        #             (node_data["entity_type"] not in ["PERSON", "LOCATION", "OBJECT", "EVENT"])
        #             # or (node_data["entity_type"] == "PERSON" and node_data["entity_name"] != "I") 
        #             )
        #     ]
        #     if not filtered_list:
        #         # 如果过滤后列表为空，标记为删除
        ...
        #         should_remove = (
        #             (src_type == "LOCATION" and tgt_type == "LOCATION") or
        #             (tgt_type == "OBJECT" and tgt_type == "OBJECT") or
        #             ((src_type == "PERSON" and tgt_type == "LOCATION") or
        #              (src_type == "LOCATION" and tgt_type == "PERSON"))
        #         )
        ...
        print(
            f"{now_ticks} Processed {already_processed}/{len(ordered_chunks)} chunks\r",
            end="",
            flush=True,
        )
    ...
    # 使用 asyncio.gather 并行处理所有chunks
    logger.info(f"[Batch Entity Extraction] Processing {len(ordered_chunks)} chunks in parallel...")
    ...
    logger.info(f"[Batch Entity Extraction] Completed: {len(all_entities_data)} entities, {len(all_edges_data)} relations")
    return knowledge_graph_inst, all_entities_data, all_edges_data
...
async def streaming_videorag_query(
    ...
    def time_str_to_seconds(day, time_str):
        ...
        # 处理可能包含小数秒的情况（如 "03.5"）
        if len(time_parts) > 2:
            second = int(float(time_parts[2]))  # 先转float再转int，处理小数秒
        else:
            second = 0
    ...
    def filter_items_by_service_type(items, time_key, service_type, key_day, 
        ...
        for item in items:
            ...
            # 计算时间差（chunk在time_key之前，所以是key_end_sec - span_end_sec）
            time_diff = key_end_sec - span_end_sec
            
            # 只要item的时间在time_key之前（time_diff >= 0），就保留
            # 由于前面已经检查了 span_end_sec <= key_end_sec，所以 time_diff 总是 >= 0
            if time_diff > 0:
        ...
        if len(filtered_items) == 0:
            if original_items is not None:
                logger.warning(f"No items found before time_key {time_key}, using original items")
                return original_items, False
            else:
                logger.warning(f"No items found before time_key {time_key}, returning empty list")
                return [], True
        else:
            logger.info(f"Filtered items by time_key {time_key}: {len(filtered_items)} items remain (from {len(items)})")
    ...
    if args.caption_retrieval:
        ...
        other_second_chunks = second_chunks[:query_param.second_top_k]     # 找到second_chunks中相似度最高的一些
        if args.multiscale:
            all_second_chunks = list(filtered_second_chunks + other_second_chunks)      # 这里我们除了分层检索了一些相似度高的second_chunks外，还保留了原本相似度高的second_chunks
        else:
            all_second_chunks = list(other_second_chunks)
    
        # 对 all_second_chunks 进行去重：如果两个 chunks 的 time_span 完全一样，则只保留一个
        seen_time_spans = set()
        deduplicated_second_chunks = []
        for chunk in all_second_chunks:
            # 获取 time_span，可能是列表或字符串
            time_span = chunk.get("time_span", [])
            if isinstance(time_span, list) and len(time_span) > 0:
                time_span_key = tuple(time_span)  # 转换为元组以便作为 set 的键
            elif isinstance(time_span, str):
                time_span_key = time_span
            else:
                time_span_key = str(time_span)  # 其他情况转为字符串
            
            if time_span_key not in seen_time_spans:
                seen_time_spans.add(time_span_key)
                deduplicated_second_chunks.append(chunk)
        
        logger.info(f"Deduplicated second chunks: {len(all_second_chunks)} -> {len(deduplicated_second_chunks)} chunks")
        ...
        # 分别对每个级别进行 token 限制
        sections = []
        # High-level Captions (hour)
        if datasets_type != "egoschema" and args.multiscale:
            if high_level_chunks:
                maybe_trun_high = truncate_list_by_token_size(
                    high_level_chunks,
                    key=lambda x: x["content"],
                    max_token_size=query_param.naive_max_token_for_text_unit // 4,  # 分配1/3的token给high-level
                )
                if maybe_trun_high:
                    high_content_list = []
                    for c in maybe_trun_high:
                        time_span = c["time_span"][0].split("_")[0]  # #-##:##:##-##:##:##
                        # 将时间戳格式从 "1-19:02:42-20:06:57" 转换为 "DAY1-19:02:42-20:06:57"
                        time_span = "DAY" + time_span.split("-")[0] + "-" + time_span.split("-")[1] + "-" + time_span.split("-")[2]
                        high_content_list.append(f"{time_span}: {c['content']}")
                    high_content = "\n".join(high_content_list)
                    sections.append(f"-----Retrieved High-level Captions-----\n{high_content}\n")
                    logger.info(f"High-level: {len(high_level_chunks)} -> {len(maybe_trun_high)} chunks")
            
            # Mid-level Captions (minute)
            if mid_level_chunks:
                maybe_trun_mid = truncate_list_by_token_size(
                    mid_level_chunks,
                    key=lambda x: x["content"],
                    max_token_size=query_param.naive_max_token_for_text_unit // 4,  # 分配1/3的token给mid-level
                )
                if maybe_trun_mid:
                    mid_content_list = []
                    for c in maybe_trun_mid:
                        time_span = c["time_span"][0].split("_")[0]  # #-##:##:##-##:##:##
                        time_span = "DAY" + time_span.split("-")[0] + "-" + time_span.split("-")[1] + "-" + time_span.split("-")[2]
                        mid_content_list.append(f"{time_span}: {c['content']}")
                    mid_content = "\n".join(mid_content_list)
                    sections.append(f"-----Retrieved Mid-level Captions-----\n{mid_content}")
                    logger.info(f"Mid-level: {len(mid_level_chunks)} -> {len(maybe_trun_mid)} chunks")
        
        # Low-level Captions (second)
        if low_level_chunks:
            maybe_trun_low = truncate_list_by_token_size(
                low_level_chunks,
                key=lambda x: x["content"],
                max_token_size=query_param.naive_max_token_for_text_unit,  # 分配1/3的token给low-level
            )
            if maybe_trun_low:
                low_content_list = []
                for c in maybe_trun_low:
                    time_span = c["time_span"][0].split("_")[0]  # #-##:##:##-##:##:##
                    time_span = "DAY" + time_span.split("-")[0] + "-" + time_span.split("-")[1] + "-" + time_span.split("-")[2]
                    low_content_list.append(f"{time_span}: {c['content']}")
                low_content = "\n".join(low_content_list)
                sections.append(f"-----Retrieved Low-level Captions-----\n{low_content}\n")
                logger.info(f"Low-level: {len(low_level_chunks)} -> {len(maybe_trun_low)} chunks")
    ...
    # entity retrieval   在知识图谱中检索实体关系对应的视频片段
    entity_retrieved_segments = set()
    if args.entity_retrieval:
        if datasets_type == "egoschema" or datasets_type == "eyewo":
    ...
    if args.visual_retrieval:
        ...
        segment_results = await video_segment_feature_vdb.query(query_for_visual_retrieval)      # 检索出最相似的视觉片
    ...
    if retrieved_segments != []:
        ...
        # visual retrieval 这一部分暂时先去掉，因为提取关键词并根据关键词重写caption感觉比较耗时而且没有必要
        # if reconstruct_caption:
        if args.reconstruct_caption:
            ...
            # )
                ...
                caption_results = retrieved_segment_caption_qwen3_api(
                        caption_model,
                        caption_tokenizer,
                        keywords_for_caption,
                        remain_segments,
            ...
            caption_results = [video_segments._data[key.split("_")[0]]["content"] for key in remain_segments]
        ...
        if isinstance(caption_results, dict):
            ...
            for timestamp_key, caption_content in caption_results.items():
                formatted_captions.append(f"Timestamp: {timestamp_key.split('_')[0]}\nContent: {caption_content}\n")
...
async def streaming_egolife_query(
    query,
    time_key, 
    service_type, 
    sub_service_type, 
    ...
    # 解析时间范围格式的time_key: 'DAY1-12:00:00-DAY3-12:00:00'
    def parse_time_range(time_range_str):
        """
        解析时间范围格式: 'DAY1-12:00:00-DAY3-12:00:00'
        
        Returns:
            tuple: (start_day, start_time, end_day, end_time) 或 (None, None, None, None) 如果解析失败
        """
        ...
        # 检查是否是新的时间范围格式（包含两个DAY）
        if time_range_str.startswith('DAY') and time_range_str.count('DAY') >= 2:
            # 找到第二个DAY的位置
            first_day_end = time_range_str.find('-', 3)  # 跳过第一个DAY
            if first_day_end == -1:
                return None, None, None, None
            
            # 找到第二个DAY的位置
            second_day_start = time_range_str.find('DAY', first_day_end)
            if second_day_start == -1:
                return None, None, None, None
            
            # 解析第一个时间点: DAY1-12:00:00
            first_part = time_range_str[:second_day_start].rstrip('-')
            first_parts = first_part.split('-', 1)
            if len(first_parts) < 2:
                return None, None, None, None
            
            start_day_str = first_parts[0]  # DAY1
            start_time_str = first_parts[1]  # 12:00:00
            
            # 提取day数字
            if start_day_str.startswith('DAY'):
                start_day = int(start_day_str[3:])
            else:
                return None, None, None, None
            
            # 解析第二个时间点: DAY3-12:00:00
            second_part = time_range_str[second_day_start:]
            second_parts = second_part.split('-', 1)
            if len(second_parts) < 2:
                return None, None, None, None
            
            end_day_str = second_parts[0]  # DAY3
            end_time_str = second_parts[1]  # 12:00:00
            
            # 提取day数字
            if end_day_str.startswith('DAY'):
                end_day = int(end_day_str[3:])
            else:
                return None, None, None, None
            
            return start_day, start_time_str, end_day, end_time_str
        else:
            # 尝试使用旧的parse_time_span格式解析（向后兼容）
            day, start_time, end_time = parse_time_span(time_range_str)
            if day is not None:
    ...
    def filter_items_by_service_type(items, time_key, service_type, key_day, 
        ...
        """
        根据service_type和time_key过滤items（可以是segments或chunks等）
        现在支持时间范围格式的time_key，提取时间跨度在time_key范围内的items
        
        Args:
            items: 要过滤的items列表（可以是segment IDs或chunks等）
            time_key: 时间键字符串，格式如 'DAY1-12:00:00-DAY3-12:00:00' (新格式) 或 'Day2 10:09:03-10:09:15' (旧格式)
            service_type: 服务类型，如 "Short-Term Proactive Service"
            key_day: 解析后的day值（从time_key解析得到，对于新格式可能为None）
            get_time_span_func: 可选函数，用于从item中提取time_span字符串
                               如果为None，则假设items本身就是time_span字符串列表
            original_items: 原始items列表，用于在过滤后为空时回退
        ...
        """
        # 首先尝试解析为时间范围格式
        key_start_day, key_start_time, key_end_day, key_end_time = parse_time_range(time_key)
        
        # 如果解析失败，尝试使用旧的格式（向后兼容）
        if key_start_day is None:
            if key_day is None:
                return items, False
            ...
            key_start_day = key_day_parsed
            key_start_time = key_start
            key_end_day = key_day_parsed
            key_end_time = key_end
        
        # 计算时间范围的开始和结束秒数
        key_start_sec = time_str_to_seconds(key_start_day, key_start_time)
        key_end_sec = time_str_to_seconds(key_end_day, key_end_time)
        ...
        for item in items:
            ...
            # 解析time_span
            span_day, span_start, span_end = parse_time_span(time_span_str)
            if span_day is None or span_start is None or span_end is None:
                continue
            
            # 计算chunk的开始和结束时间（秒数）
            span_start_sec = time_str_to_seconds(span_day, span_start)
            span_end_sec = time_str_to_seconds(span_day, span_end)
            
            # 检查chunk的时间跨度是否与time_key范围有重叠
            # 有重叠的条件：chunk的开始时间 <= time_key的结束时间 且 chunk的结束时间 >= time_key的开始时间
            if span_end_sec <= key_end_sec and span_start_sec >= key_start_sec:
                filtered_items.append(item)
        
        # 如果过滤后为空，回退到原始items
        if len(filtered_items) == 0:
            if original_items is not None:
                logger.warning(f"No items found in time_key range {time_key}, using original items")
                return original_items, False
            else:
                logger.warning(f"No items found in time_key range {time_key}, returning empty list")
                return [], True
        else:
            logger.info(f"Filtered items by time_key range {time_key}: {len(filtered_items)} items remain (from {len(items)})")
            return filtered_items, True
    ...
    # 首先尝试解析为时间范围格式
    key_start_day, key_start_time, key_end_day, key_end_time = parse_time_range(time_key)
    if key_start_day is not None:
        # 新格式：使用开始时间作为key_day（用于向后兼容）
        key_day = key_start_day
        key_start = key_start_time
        key_end = key_end_time
    else:
        # 旧格式：使用parse_time_span解析
        key_day, key_start, key_end = parse_time_span(time_key)
