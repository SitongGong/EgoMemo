    ...
    def generate_proactive_service(
        ...
        if self.high_level_category == "egolife_long_term":
            ...
            for group_idx, (group_key, json_group) in enumerate(json_contents.items(), 1):
                ...
                #         json_content_text = json_content_text + f"\n\n ### Historical Habit Summary JSON ### \n\n {json.dumps(accumulated_results["habit_events"], ensure_ascii=False, indent=2)}"
                #     else:
                #         json_content_text = json_content_text + f"\n\n ### Historical Habit Summary JSON ### \n\n No historical habit summary available"
                # elif self.low_level_category == "memory_link_contextual":
                #     if accumulated_results["memory_events"]:
                #         json_content_text = json_content_text + f"\n\n ### Historical Memory Summary JSON ### \n\n {json.dumps(accumulated_results["memory_events"], ensure_ascii=False, indent=2)}"
                #     else:
                #         json_content_text = json_content_text + f"\n\n ### Historical Memory Summary JSON ### \n\n No historical memory summary available"
                # elif self.low_level_category == "routine_optimization":
                #     if accumulated_results["routine_events"]:
                #         json_content_text = json_content_text + f"\n\n ### Historical Routine Summary JSON ### \n\n {json.dumps(accumulated_results["routine_events"], ensure_ascii=False, indent=2)}"
                #     else:
                #         json_content_text = json_content_text + f"\n\n ### Historical Routine Summary JSON ### \n\n No historical routine summary available"
                # elif self.low_level_category == "personal_progressive":
                #     if accumulated_results["progress_events"]:
                #         json_content_text = json_content_text + f"\n\n ### Historical Progress Summary JSON ### \n\n {json.dumps(accumulated_results["progress_events"], ensure_ascii=False, indent=2)}"
                #     else:
                #         json_content_text = json_content_text + f"\n\n ### Historical Progress Summary JSON ### \n\n No historical progress summary available"
        ...
        # 如果是其他几种类型的服务，换成inline或file请求
            ...
            # 第五步：获取结果
            print("开始分批处理请求...")
            for batch_idx in range(2, total_batches):      # total_batches
                start_idx = batch_idx * batch_size
                end_idx = min(start_idx + batch_size, len(inline_requests))
                
                print(f"\n处理第 {batch_idx + 1}/{total_batches} 批请求 (索引 {start_idx} 到 {end_idx - 1})...")
                
                # 根据 input_mode 选择请求方式
                uploaded_file = None
                if self.input_mode == "file":
                    # 准备文件请求
                    batch_file_requests = file_requests[start_idx: end_idx]
                    # 创建临时文件
                    temp_file_name = f"batch-requests-{batch_idx + 1}.jsonl"
                    with open(temp_file_name, "w", encoding="utf-8") as f:
                        for request_item in batch_file_requests:
                            f.write(json.dumps(request_item, ensure_ascii=False) + "\n")
                    
                    # 上传文件到 File API
                    uploaded_file = self.batch_client.files.upload(
                        file=temp_file_name,
                        config=types.UploadFileConfig(display_name=f'batch-requests-{batch_idx + 1}', mime_type='jsonl')
                    )
                ...
                try:
                    if self.input_mode == "inline":
                        ...
                        raise ValueError(f"不支持的 input_mode: {self.input_mode}")
                ...
                try:
                    while True:
                        ...
                        if batch_job.state.name == "JOB_STATE_SUCCEEDED":
                            ...
                            error_msg = f"Batch job 失败: {batch_job.name}, 状态: {batch_job.state.name}"
                            # 如果是超时错误，保存已处理的结果
                            if batch_job.state.name == "JOB_STATE_EXPIRED" or "Deadline expired" in str(batch_job.state):
                                print(f"警告: {error_msg}")
                                if output_path and conversation_results:
                                    try:
                                        os.makedirs(output_path, exist_ok=True)
                                        save_path = os.path.join(output_path, "conversation_results_partial.json")
                                        with open(save_path, "w", encoding="utf-8") as f:
                                        ...
                                        print(f"警告: 保存部分结果时出错: {save_error}")
                    ...
                    error_str = str(e)
                    # 检查是否是超时错误
                    if "Deadline expired" in error_str or "deadline" in error_str.lower():
                        print(f"警告: 遇到超时错误: {error_str}")
                        if output_path and conversation_results:
                            try:
                                os.makedirs(output_path, exist_ok=True)
                                save_path = os.path.join(output_path, "conversation_results_partial.json")
                ...
                try:
                    ...
                    if self.input_mode == "inline":
                        ...
                        # 按原始请求顺序构建响应列表（使用 key 匹配）
                        # 注意：file_requests 中的 key 是 "request-{group_idx}"，group_idx 从1开始
                        responses = []
                        for local_idx in range(end_idx - start_idx):
                            group_idx = start_idx + local_idx + 1  # group_idx 从1开始
                            key = f"request-{group_idx}"
                            if key in indexed_results:
                                parsed_result = indexed_results[key]
                                # 创建一个类似 inline_response 的对象
                                class FileResponse:
                                    def __init__(self, result):
                                        if result:
                                            if isinstance(result, str):
                                                self.response = type('obj', (object,), {'text': result})()
                                            elif isinstance(result, dict):
                                                self.response = type('obj', (object,), {'text': json.dumps(result, ensure_ascii=False)})()
                                            else:
                                                self.response = type('obj', (object,), {'text': str(result)})()
                                        else:
                                            self.response = None
                                        self.error = None
                                responses.append(FileResponse(parsed_result))
                            else:
                                # 如果找不到对应的结果，创建一个空响应
                                class FileResponse:
                                    def __init__(self):
                                        self.response = None
                                        self.error = f"Result not found for key: {key}"
                                responses.append(FileResponse())
                                print(f"警告: 缺少 key '{key}' 的结果")
                    else:
                        raise ValueError(f"不支持的 input_mode: {self.input_mode}")
                    ...
                    traceback.print_exc()
                    # 检查是否是超时错误
                    is_deadline_error = "Deadline expired" in error_str or "deadline" in error_str.lower() or "expired" in error_str.lower()
                    if is_deadline_error:
                        print(f"警告: 遇到超时错误 (Deadline expired)，将保存已处理的结果")
                    # 保存当前已处理的结果
                    if output_path and conversation_results:
                        try:
                            os.makedirs(output_path, exist_ok=True)
                            ...
                            print(f"警告: 保存部分结果时出错: {save_error}")
                    # 如果是超时错误，返回已处理的结果而不是抛出异常
                    if is_deadline_error:
                        print(f"由于超时错误，返回已处理的结果，共处理了 {batch_idx} 批数据")
...
def main(args):
    ...
    try:
        ...
        if args.conversation_stage:
            output_path = os.path.join(args.output_path, args.high_level_service_type, args.low_level_service_type)
            os.makedirs(output_path, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parse.add_argument("--activity_list", type=list, default=["A4_LUCIA", "A5_KATRINA", "A6_SHURE"])
    ...
    # 对于服务类型prompt的选择
    parse.add_argument("--high_level_service_type", type=str, default="egolife_episodic")
    parse.add_argument("--low_level_service_type", type=str, default="memory_recall")
